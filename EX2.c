...
int main() {
    ...
    while (1) {
        ...
        if (record_no < 0 || record_no >= NUM_RECORDS ||quit==TRUE) //
            break;
        ...
        switch (operation) {
            ...
            case 'd' : // deposit
                ...
                pos = record_no * sizeof(struct record);
                lseek(fd, pos, SEEK_SET);
                n = read(fd, &current, sizeof(struct record)); display_record (&current);
                printf("enter amount\n");
                scanf("%d", &amount);
            ...
            case 'w':
				...
				break;
            case 't':
                printf("송금 할 번호 입력(0-99) : ");
				scanf("%d", &record_no2);

				reclock(fd, record_no, sizeof(struct record), F_WRLCK); //이체 금액 빼주기
				pos = record_no*sizeof(struct record);
				lseek(fd, pos, SEEK_SET);
				n = read(fd, &current, sizeof(struct record));
				printf("enter amount\n");
				...
				reclock(fd, record_no2, sizeof(struct record), F_WRLCK); //이체 금액 채우기
				pos = record_no2*sizeof(struct record);
				lseek(fd, pos, SEEK_SET);
				n = read(fd, &current, sizeof(struct record));
				current.balance += amount;
				lseek(fd, pos, SEEK_SET);
				write(fd, &current, sizeof(struct record));
				reclock(fd, record_no2, sizeof(struct record), F_UNLCK);
				break;
            case 'q' :
                    quit = TRUE;
                    break;
